...
- Lanes (persistent reused worktrees) make acquire-time the second never-lose-work site: salvage a dirty lane to a WIP branch BEFORE `reset --hard`/`clean -fd`, and refuse the reset if the salvage fails. Clean with `-fd`, never `-x` (warm node_modules/caches are the point); worktrees are born without git-ignored files, so `.worktreeinclude` declares what gets copied in (copy, never symlink).
