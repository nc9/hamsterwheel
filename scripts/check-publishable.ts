...
const PKGS = ["packages/config", "packages/gate", "packages/runners", "packages/sandbox", "apps/cli"];
let bad = false;
...
for (const dir of PKGS) {
  ...
  if (!p.license) { console.error(`✗ ${p.name}: no license field`); bad = true; }
  if (p.name.startsWith("@") && p.publishConfig?.access !== "public") {
    console.error(`✗ ${p.name}: scoped package without publishConfig.access="public" will publish restricted`);
    bad = true;
  }
}
