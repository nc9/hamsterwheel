...
- **It yields to people.** An issue someone has assigned, commented on from outside the org, or labelled `loop:hands-off` is parked for a human instead of worked. On a public repo the comment is the load-bearing signal: a drive-by contributor has no write access, so "I'd like to work on this" is the only way they can call it. The check is deliberately blunt, because a false positive costs a maintainer seconds and a false negative costs a contributor their afternoon.
